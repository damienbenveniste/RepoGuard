"""Command line interface for repo-guard."""
...
from repo_guard.checks.base import CheckConfigurationError, CheckReport
from repo_guard.checks.runner import run_checks
from repo_guard.compile_rules import CompileRulesSummary, compile_rules
from repo_guard.diffing import DiffInspectionError, DiffReport, inspect_diff
from repo_guard.doctor import DoctorReport, run_doctor
from repo_guard.models import ScaffoldSummary
from repo_guard.project_config import ProjectConfigError
from repo_guard.scaffold import build_init_options, scaffold_package_project
from repo_guard.validation import ValidationError, ValidationReport, run_validation
...
def _print_init_summary(summary: ScaffoldSummary, *, agent: AgentOption) -> None:
    ...
    typer.echo(f"{action} RepoGuard Python project: {summary.target_dir.name}")
    typer.echo()
    typer.echo("Files:")
    ...
    typer.echo("  uv run repo-guard check")
    typer.echo("  uv run repo-guard validate")


def _print_check_report(report: CheckReport) -> None:
    """Print a human-readable check report."""
    typer.echo(f"repo-guard check: {'ok' if report.ok else 'failed'}")
...
def _print_validation_report(report: ValidationReport) -> None:
    ...
    typer.echo(f"repo-guard validate: {'ok' if report.ok else 'failed'}")
    typer.echo(f"path: {report.path}")
...
def _print_doctor_report(report: DoctorReport) -> None:
    ...
    typer.echo(f"repo-guard doctor: {'ok' if report.ok else 'failed'}")
    typer.echo(f"path: {report.path}")
    for check in report.checks:
...
def version_command() -> None:
    """Print the installed repo-guard version."""
...
def init_command(
    ...
    """Create a new RepoGuard Python project."""
    try:
...
def compile_rules_command(
    path: Annotated[
        Path,
        typer.Option("--path", help="Project root containing repo-guard.toml.", resolve_path=True),
    ] = Path(),
    agent: Annotated[
        AgentOption,
