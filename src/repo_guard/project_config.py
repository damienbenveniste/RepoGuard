...
class GeneratedProjectConfig:
    """Parsed subset of `repo-guard.toml` used by V1 commands."""
...
def load_generated_project_config(root: Path) -> GeneratedProjectConfig:
    """Load and validate required `repo-guard.toml` fields."""
    resolved_root = root.resolve(strict=False)
    config_path = resolved_root / "repo-guard.toml"
    if not config_path.exists():
        msg = f"Generated project config is missing: {config_path}"
        raise ProjectConfigError(msg)

    config = load_repo_guard_toml(resolved_root)
