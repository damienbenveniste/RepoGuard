...
from repo_guard.checks.base import CheckFinding, CheckResult, finding
from repo_guard.checks.config import (
    bool_value,
    int_value,
    load_repo_guard_toml,
    str_value,
    table_value,
)
...
def check_config_consistency(root: Path) -> CheckResult:
    ...
    config_path = root / "repo-guard.toml"
    ...
    if not config_path.exists():
        findings.append(
            finding(
                "repo-guard.toml",
                line=0,
                code="missing-repo-guard-config",
                message="Generated projects must include repo-guard.toml.",
            )
        )
        return CheckResult(id="config-consistency", findings=tuple(findings))

    config = load_repo_guard_toml(root)
    ...
    if bool_value(agent_table, "codex", default=True) and not (root / "AGENTS.md").exists():
        findings.append(
            finding(
                ...
                message="repo-guard.toml enables Codex but AGENTS.md is missing.",
            )
        )
    if bool_value(agent_table, "claude", default=False) != (root / "CLAUDE.md").exists():
        findings.append(
            finding(
                ...
                line=0,
                code="agent-config-mismatch",
                message="repo-guard.toml Claude setting does not match CLAUDE.md.",
            )
        )
    if bool_value(agent_table, "cursor", default=False) != (root / ".cursor/rules").exists():
        findings.append(
            finding(
                ...
                line=0,
                code="agent-config-mismatch",
                message="repo-guard.toml Cursor setting does not match .cursor/rules.",
            )
        )
    return findings
...
def _check_coverage(project: object, pyproject_text: str) -> list[CheckFinding]:
    """Verify coverage config matches `repo-guard.toml`."""
    ...
    return [
        finding(
            ...
            message="coverage_fail_under in repo-guard.toml must match pyproject.toml.",
        )
    ]
...
def _check_python_min(project: object, pyproject_text: str) -> list[CheckFinding]:
    ...
    return [
        finding(
            ...
            message="python_min in repo-guard.toml must match pyproject.toml.",
        )
    ]
